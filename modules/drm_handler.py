...
import modules.saini as helper  # fixed: package-qualified
from modules import globals      # fixed: package-qualified
from modules.utils import progress_bar  # fixed: package-qualified
...
async def drm_handler(bot: Client, m: Message):
    ...
    if m.document:
        ...
        await m.reply_text(f"**Enter Batch Name or send /d**")
        try:
            input1: Message = await bot.listen(m.chat.id, timeout=20)
    ...
    try:
        for i in range(arg - 1, len(links)):
            ...
            # URL transforms and command selection
            if "visionias" in url:
            ...
            try:
                ...
                await bot.send_message(
                    channel_id,
                    f'⚠️**Downloading Failed**⚠️\n**Name** =>> `{str(count).zfill(3)} {name1}`\n**Url** =>> {url}\n\n<blockquote expandable><i><b>Failed Reason: {str(e)}</b></i></blockquote>',
