...
from modules import globals  # fixed: package-qualified
...
def register_settings_handlers(bot: Client):
    ...
    async def thumbnail_menu(client, callback_query):
        ...
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("🎥 Video", callback_data="viideo_thumbnail_command"),
             InlineKeyboardButton("📑 PDF", callback_data="pddf_thumbnail_command")],
    ...
    async def watermark_menu(client, callback_query):
        ...
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("🎥 Video", callback_data="video_wateermark_command"),
             InlineKeyboardButton("📑 PDF", callback_data="pdf_wateermark_command")],
            [InlineKeyboardButton("🔙 Back to Settings", callback_data="setttings")]
        ])
        await callback_query.message.edit_media(
            InputMediaPhoto(
    ...
    async def handle_caption(client, callback_query):
        ...
        editable = await callback_query.message.edit(
            ...
            "**Send Your Caption Style eg. /cc1 or /cc2 or /cc3**",
            reply_markup=keyboard
        )
        ...
        try:
            if input_msg.text.lower() == "/cc1":
                globals.caption = "/cc1"
                await editable.edit("✅ Caption Style 1 Updated!", reply_markup=keyboard)
            elif input_msg.text.lower() == "/cc2":
                globals.caption = "/cc2"
                await editable.edit("✅ Caption Style 2 Updated!", reply_markup=keyboard)
            else:
                globals.caption = input_msg.text
                await editable.edit("✅ Caption Style 3 Updated!", reply_markup=keyboard)
    ...
    async def handle_filename(client, callback_query):
        ...
        try:
            if input_msg.text.lower() == "/d":
                ...
                await editable.edit("✅ End File Name Disabled !", reply_markup=keyboard)
            else:
    ...
    async def video_thumbnail(client, callback_query):
        ...
        editable = await callback_query.message.edit("Send the Video Thumb URL or Send /d \n<blockquote><b>Note </b>- For document format send : No</blockquote>", reply_markup=keyboard)
        input_msg = await bot.listen(editable.chat.id)
        try:
            if input_msg.text.startswith("http://") or input_msg.text.startswith("https://"):
                globals.thumb = input_msg.text
                await editable.edit("✅ Thumbnail set successfully from the URL !", reply_markup=keyboard)
            elif input_msg.text.lower() == "/d":
                globals.thumb = "/d"
                await editable.edit("✅ Thumbnail set to default !", reply_markup=keyboard)
            else:
                globals.thumb = input_msg.text
                await editable.edit("✅ Video in Document Format is enabled !", reply_markup=keyboard)
        except Exception as e:
    ...
    async def credit(client, callback_query):
        user_id = callback_query.from_user.id
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back to Settings", callback_data="setttings")]])
        editable = await callback_query.message.edit("Send Credit Name or Send /d", reply_markup=keyboard)
        input_msg = await bot.listen(editable.chat.id)
        try:
            if input_msg.text.lower() == "/d":
                globals.CR = f"{CREDIT}"
                await editable.edit("✅ Credit set to default !", reply_markup=keyboard)
            else:
    ...
    async def handle_cw_token(client, callback_query):
        ...
        try:
            if input_msg.text.lower() == "/d":
                ...
                await editable.edit("✅ Carrerwill Token set successfully as default !", reply_markup=keyboard)
            else:
    ...
    async def video_watermark(client, callback_query):
        ...
        editable = await callback_query.message.edit("**Send Video Watermark text or Send /d**", reply_markup=keyboard)
        input_msg = await bot.listen(editable.chat.id)
        try:
            if input_msg.text.lower() == "/d":
                globals.vidwatermark = "/d"
                await editable.edit("**Video Watermark Disabled ✅** !", reply_markup=keyboard)
            else:
    ...
    async def topic_toggle(client, callback_query):
        user_id = callback_query.from_user.id
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back to Settings", callback_data="setttings")]])
        editable = await callback_query.message.edit("**If you want to enable topic in caption: send /yes or send /d**\n\n<blockquote><b>Topic fetch from (bracket) in title</b></blockquote>", reply_markup=keyboard)
        input_msg = await bot.listen(editable.chat.id)
        try:
            if input_msg.text.lower() == "/yes":
                globals.topic = "/yes"
                await editable.edit("**Topic enabled in Caption ✅** !", reply_markup=keyboard)
            else:
                globals.topic = input_msg.text
                await editable.edit("Topic disabled in Caption ✅!", reply_markup=keyboard)
        except Exception as e:
    ...
    async def reset_settings(client, callback_query):
        user_id = callback_query.from_user.id
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back to Settings", callback_data="setttings")]])
        editable = await callback_query.message.edit("If you want to reset settings send /yes or Send /no", reply_markup=keyboard)
        input_msg = await bot.listen(editable.chat.id)
        try:
            if input_msg.text.lower() == "/yes":
                globals.caption = "/cc1"
                ...
                globals.res = "854x480"
                globals.topic = "/d"
                await editable.edit("✅ Settings reset as default !", reply_markup=keyboard)
            else:
                await editable.edit("✅ Settings Not Changed !", reply_markup=keyboard)
        except Exception as e:
